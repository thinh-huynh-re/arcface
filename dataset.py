...
def dali_data_iter(
    ...
    size_resize = fn.random.uniform(
        range=(int(112 * 0.5), int(112 * 0.8)), dtype=types.FLOAT
    )
    ...
    with pipe:
        ...
        if dali_aug:
            ...
            images = multiplexing(
                condition_resize,
                dali_random_resize(images, size_resize, image_size=112),
                images,
            )
            images = multiplexing(
                condition_blur,
                dali_random_gaussian_blur(images, window_size_blur),
                images,
            )
            images = multiplexing(
                condition_hsv, dali_random_hsv(images, hsv_hue, hsv_saturation), images
            )
